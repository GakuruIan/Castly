...
export const PollSchema = yup.object().shape({
  ...
  description: yup
    .string()
    .min(5, "Description cannot be less then 5 characters")
  ...
  options: yup
    ...
    .of(
      yup.object({
        option: yup
          .string()
          .min(3, "option cannot be less than 3 characters")
          .max(30, "Option cannot be more than 30 characters")
  ...
  closeDate: yup
    ...
    .when("openDate", (openDateValue, schema) => {
      ...
      const openDate = Array.isArray(openDateValue)
        ? openDateValue[0]
        : openDateValue;

      return openDate
        ? schema.min(openDate, "Close date cannot be earlier than open date")
        : schema;
    }),
  requireParcipantName: yup
    .string()
    .min(3, "Name cannot be less than 3 characters"),
});
